            ...
            {lists.map((list, key) => {
              ...
              return (
                <li
                  ...
                  role="tab"
                  tabIndex={list.id === selectListId ? 0 : -1}
...
const Tasks = (props) => {
  ...
  const calculateLimitLeft = (_limit) => {
    const now = new Date();
    // new Dateは実行環境元の時刻元にする。現状UTCになるためJSTに変換
    // now.setHours(now.getHours() + 9);
    const limit = new Date(_limit);
    ...
    return `期限まで${dayLeft}日${hourLeft}時間${minutesLeft}分`;
  };
        ...
        .map((task, key) => (
          <li key={key} className="task-item">
            ...
            >
              ...
              {task.done ? "完了" : "未完了"}
              <br />
              残り日時: {calculateLimitLeft(task.limit)}
            </Link>
          </li>
        ))}
    </ul>
